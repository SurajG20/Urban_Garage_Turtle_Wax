...
function SellCarUser() {
  ...
  const deleteSellCarUserMutation = useMutation(
    (id) => {
      ...
      return axios.delete(`${import.meta.env.VITE_API_URL}/sellCar-users/${id}`);
    },
    {
      onSuccess: () => {
              ...
              <div className='relative overflow-x-auto shadow-md sm:rounded-lg'>
                <table className='w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400'>
                  <thead className='text-xs text-gray-800 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400'>
                    <tr>
                      <th scope='col' className='px-6 py-3 text-center flex items-center'>
                      ...
                      <th scope='col' className='px-6 py-3 flex items-center '>
                        <BsFillFuelPumpFill className='text-theme-red mr-1' /> Fuel Type/{' '}
                    ...
                    {isLoading ? (
                      ...
                      data.map((users) => (
                        ...
                        >
                          ...
                          <td className='px-6 py-4 text-gray-700 text-center'>{users.fuelType}</td>
                          <td className='px-6 py-4 text-gray-900 flex items-center text-xs'>
                            {users.regCity}/{users.address ? users.address.slice(0, 12) : 'N/A'}
                            ...
                          </td>
